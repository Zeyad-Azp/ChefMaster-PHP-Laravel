...
<<<<<<< HEAD
    ...
    if (res.status === 401) {
        ...
        const redirect = body.redirect || '/login';
        const message  = body.message  || 'Please log in to continue.';
        if (action !== 'get_recipes' && action !== 'get_recipe') {
...
<<<<<<< HEAD
            ...
            if (!res.message?.toLowerCase().includes('log in') &&
        ...
        // Non-JSON response = unauthenticated redirect — show empty state quietly
        State.recipes = [];
        updateRecipesView(activeFilter);
        updateFavoritesView();
=======
...
<<<<<<< HEAD
    ...
    function clearErrorByName(name) {
...
<<<<<<< HEAD
        ...
        if (saveRes.redirect) {
            ...
            setTimeout(() => { window.location.href = saveRes.redirect; }, 1500);
            if (btn) { btn.disabled = false; btn.innerHTML = isFavorite ? Icon.heart : Icon.save; }
            return;
        }
        ...
        if (saveRes.success) {
            ...
            const existingIdx = State.recipes.findIndex(r => r.api_recipe_id == recipeId);
            if (existingIdx > -1) {
                State.recipes[existingIdx] = saveRes.data;
            } else {
